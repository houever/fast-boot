...
import cn.fastboot.base.annotations.CurrentUser;
...
//        return object;
        CurrentUser currentUserAnnotation = methodParameter.getParameterAnnotation(CurrentUser.class);
//        CurrentUser oAuth2AccessToken = GsonUtil.gsonToBean(nativeWebRequest.getHeader(SecurityConstants.AUTH_HEADER), OAuth2AccessToken.class);
        log.info("methodParameter:{}",methodParameter);
        log.info("NativeWebRequest:{}",nativeWebRequest);
        log.info("CurrentUser:{}",currentUserAnnotation);
        return null;
    }
}
